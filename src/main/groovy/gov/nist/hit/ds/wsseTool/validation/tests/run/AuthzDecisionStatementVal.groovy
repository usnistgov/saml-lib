...
public class AuthzDecisionStatementVal extends BaseVal {
	...
	public void resource(){
		for(GPathResult authz : header.map.authzDecisionStatements){
			...
			Object param = context.getParams().get("To");
			
			assumeTrue("cannot use param provided", param instanceof String)

			String to = param.trim()
			String resource = authz.@Resource.text().trim()
			
			boolean test = resource == "" || resource == to;
			
			assertTrue(MessageFormat.format(" @Resource = <value of S:Header/wsa:To> or ''. Header was {0} and expected {1} ", resource, to), test)
		}
	}
